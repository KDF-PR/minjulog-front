// api.mapper.ts — 백엔드 원형(`api.dto.ts`) ↔ 화면 모델(`models.ts`) 변환.
//
// **왜 한 파일로 모으나** — 백엔드 응답 형태가 바뀌면 고칠 곳이 여기 하나가 되게 하려는 것이다.
// 변환이 서비스마다 흩어져 있으면 필드 하나 바뀔 때 어디를 고쳐야 하는지 찾아다녀야 한다.
//
// 규칙 세 가지
//   · 화면 모델은 DTO 필드를 그대로 베끼지 않는다. 화면이 쓰는 이름으로 바꾼다
//   · 백엔드 표기법(snake_case)이 이 파일 밖으로 새지 않는다
//   · 날짜 문자열은 여기서 `Date` 로 바꾼다. 화면이 파싱하지 않는다
