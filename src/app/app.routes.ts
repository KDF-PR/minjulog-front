...
/**
 * 라우팅. 모든 화면을 `loadComponent` 로 지연 로드.
 *
 * **둘러보기에는 가드를 붙이지 않는다.** 현장에 선 사람에게 첫 화면부터 번호를 요구하면
 * 그냥 나가버린다 → 기록을 남기는 순간에만 차단. 막힌 사람은 `/gate` 로 가고
 * 원래 주소는 `returnUrl` 로 동행 (`docs/요구사항정의.md` 5장 「로그인 시점」).
 */
export const routes: Routes = [
  ...
  // 차단 지점은 「리워드 받기」를 누르는 순간이고, 그 판단은 화면 안에서
  {
    path: 'reward',
  ...
  // 개발 확인용 · 요구사항 36개 밖. main 이 채워지면 제거
  {
    path: 'dashboard',
